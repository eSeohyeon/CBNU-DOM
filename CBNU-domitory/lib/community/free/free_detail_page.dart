...
class _FreePostDetailPageState extends State<FreePostDetailPage> {
  ...
  Future<void> _submitComment() async {
    ...
    try {
      ...
      final postRef = FirebaseFirestore.instance.collection('free_posts').doc(widget.post.postId);

      if (_replyingToCommentId == null) {
        await postRef.collection('comments').add(commentData);
      } else {
        await postRef.collection('comments').doc(_replyingToCommentId).collection('replies').add(commentData);
      }
  ...
  Future<void> _showDeleteConfirmationDialog({required String title, required String content, required VoidCallback onDelete}) async {
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(title),
          content: SingleChildScrollView(child: ListBody(children: <Widget>[Text(content)])),
          actions: <Widget>[
            TextButton(
              child: const Text('취소'),
    ...
    await _showDeleteConfirmationDialog(
      ...
      onDelete: () async {
        try {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('게시글 삭제에 실패했습니다: $e')));
          }
        }
      },
    );
    ...
    await _showDeleteConfirmationDialog(
      ...
      onDelete: () async {
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('댓글 삭제에 실패했습니다: $e')));
        }
      },
    );
  }
    ...
    await _showDeleteConfirmationDialog(
      ...
      onDelete: () async {
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('대댓글 삭제에 실패했습니다: $e')));
        }
      },
    );
  }
          ...
          actions: [
            PopupMenuButton<String>(
              ...
              itemBuilder: (BuildContext context) {
                ...
                if (isMyPost) {
                  items.add(PopupMenuItem(value: 'delete', child: Text('삭제', style: mediumBlack16)));
                } else {
                  items.add(PopupMenuItem(value: 'dm', child: Text('1:1 쪽지', style: mediumBlack16)));
                  items.add(PopupMenuItem(value: 'report', child: Text('신고', style: mediumBlack16)));
                }
                return items;
              },
                      ...
                      Padding(
                        ...
                        child: Column(
                            ...
                            children: [
                              Row(
                                  children: [
                                    SizedBox(width: 36.w, height: 36.h, child: Image.asset('assets/profile_man.png')),
                                    SizedBox(width: 6.w),
                                    Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        mainAxisSize: MainAxisSize.min,
                                  ...
                                  FutureBuilder<int>(
                                    future: () async {
                                      ...
                                      for (final commentDoc in comments) {
                                        final replySnapshot = await commentDoc.reference.collection('replies').count().get();
                                        totalCount += replySnapshot.count ?? 0;
                                      }
                                      return totalCount;
                                    }(),
                                    builder: (context, countSnapshot) {
                                      return Text('${countSnapshot.data ?? comments.length}', style: mediumGrey14);
                                    },
                                  )
                                ]),
                              ),
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          InkWell(onTap: _cancelReplying, child: Icon(Icons.close, size: 16, color: grey))
                        ],
                      ),
                    ),
                  Padding(
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
          Row(children: [
            SizedBox(width: 28.w, height: 28.h, child: Image.asset('assets/profile_man.png')),
            SizedBox(width: 8.w),
          ...
          SizedBox(height: 8.h),
          Padding(padding: EdgeInsets.only(left: 36.w), child: Text(data['contents'], style: mediumBlack14)),
          Align(
          ...
          StreamBuilder<QuerySnapshot>(
            stream: commentDoc.reference.collection('replies').orderBy('createdAt').snapshots(),
            builder: (context, snapshot) {
              if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
...
class SubCommentItem extends StatelessWidget {
  ...
  const SubCommentItem({super.key, required this.replyDoc, required this.commentId, required this.onDelete});
