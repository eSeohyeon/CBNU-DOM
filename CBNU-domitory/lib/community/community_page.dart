...
import 'package:untitled/community/search_page.dart';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          shape: Border(
              bottom: BorderSide(color: grey_seperating_line, width: 1.0)),
          title: PreferredSize(
            ...
            child: TabBar(
                ...
                tabs: [Tab(text: '자유게시판'), Tab(text: '공동구매')]),
          ),
          ...
          actions: [
            IconButton(
              icon: Icon(Icons.edit,
                  color: _isStudent ? black : black40, size: 24),
              onPressed: () {
                if (_isStudent) {
                  if (_tabController.index == 0) {
                    Navigator.push(context,
                        MaterialPageRoute(builder: (context) => CreateFreePost()));
                  } else {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => GroupBuyCreatePage()));
                  }
                } else {
                  showDialog(
                      context: context,
            ...
            IconButton(
              icon: Icon(Icons.search, color: black, size: 24),
              onPressed: () {
                Navigator.push(context,
        ...
        body: TabBarView(
            controller: _tabController,
            children: [FreePostTab(), GroupBuyPostTab()]));
  }
}
